...
fn flush(store: &mut Store, pending: &mut HashMap<PathBuf, Op>) -> Result<()> {
    ...
    let tx = store.conn.transaction()?;
    {
        ...
        let mut relocate = tx.prepare(
            "UPDATE nodes SET name=?3, parent_dev=?4, parent_inode=?5, last_seen=?6
             WHERE dev_id=?1 AND inode=?2",
        )?;
        ...
        let mut del_fts = tx.prepare("DELETE FROM names_fts WHERE dev=?1 AND ino=?2")?;
        ...
        for full in &deletes {
            let r: Result<()> = (|| {
                ...
                {
                    del_subtree(&mut descendants, &mut del_fts, &mut del_node, cdev, cino)?;
        ...
        for full in &moved_to {
            let r: Result<()> = (|| {
                ...
                if let Some((_oldname, opdev, opino, _k, rb, ri)) = existing {
                    ...
                    relocate.execute(params![dev, ino, name, npdev, npino, now])?;
                    del_fts.execute(params![dev, ino])?;
                ...
                match existing {
                    ...
                    other => {
                        if let Some((edev, eino, _ek, erb, eri, _eb)) = other {
                            ...
                            del_subtree(&mut descendants, &mut del_fts, &mut del_node, edev, eino)?;
